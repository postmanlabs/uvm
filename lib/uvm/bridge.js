var vm = require('vm'),
    ...
    /**
     * Replace all instances of single quote with its escaped counterpart in string form \'
     * @param {String} str
     * @returns {String}
     */
    escapeSingleQuotes = function (str) {
        return str.replace(MATCH_SINGLEQUOTES, ESCAPED_SINGLEQUOTE);
    },
...
module.exports = function (emitter, options, callback) {
    ...
    emitter._dispatch = function () {
        try {
            ...
            vm.runInContext(`__uvm_dispatch('${escapeSingleQuotes(jsonArray(arguments))}');`, context, {
