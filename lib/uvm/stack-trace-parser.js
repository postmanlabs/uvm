...
function parseGecko (line) {
    ...
    return {
        ...
        lineNumber: parts[4] ? Number(parts[4]) : null,
...
module.exports = {
    line: function (traceString, offset = 0) {
        if (!traceString) {
            return null;
        }
        ...
        for (let i = 0, ii = lines.length; i < ii; i++) {
            let line = lines[i],
                stack = parseChrome(line) || parseWinjs(line) || parseGecko(line) || parseNode(line) || parseJSC(line);

            if (stack && Number.isFinite(stack.lineNumber) && (stack.lineNumber > offset)) {
    ...
    parse: function (stackString) {
        ...
        return lines.reduce((stack, line) => {
            const parseResult = parseChrome(line) || parseWinjs(line) || parseGecko(line) || parseNode(line) ||
                parseJSC(line);
